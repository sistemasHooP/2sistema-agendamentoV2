...
                // --- LÓGICA DE ATUALIZAÇÃO LOCAL OTIMIZADA ---
                if (this.state.data.appointments) {
                    this.state.data.appointments = this.state.data.appointments.filter(
                        appt => appt.ID_Agendamento !== appointmentId
                    );
                }
                if (this.state.appointments.data) {
                    this.state.appointments.data = this.state.appointments.data.filter(
                        appt => appt.ID_Agendamento !== appointmentId
                    );
                    this.state.appointments.totalCount--;
                }
...
                const isCurrentlyPriority = icon.classList.contains('is-priority');

                // --- LÓGICA OTIMISTA ---
                icon.classList.toggle('is-priority', !isCurrentlyPriority);
                const appointment = this.state.data.appointments.find(a => a.ID_Agendamento === id);
                if (appointment) {
                    appointment.Prioridade = isCurrentlyPriority ? '' : 'SIM';
                }
                // --- FIM ---

                try {
                    await callApi('toggleAppointmentPriority', { appointmentId: id });
                } catch (err) {
                    this.showNotification('error', 'Erro ao atualizar', err.message);
                    icon.classList.toggle('is-priority', isCurrentlyPriority);
                    if (appointment) {
                        appointment.Prioridade = isCurrentlyPriority ? 'SIM' : '';
                    }
...
                    try {
                        // --- OTIMIZAÇÃO: UMA CHAMADA À API EM VEZ DE DUAS ---
                        const response = await callApi('updateAppointmentStatus', { appointmentId: id, newStatus: 'Concluído' });
                        this.showNotification('success', 'Sucesso!', 'Atendimento concluído.');
                        this.state.data.agendaData = response.newAgendaData; // Usa o retorno da API
                        this.renderProfessionalAgenda(); // Redesenha com os dados recebidos
